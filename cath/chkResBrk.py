...
for id in domain_ids:
    ...
    for i in range(len(residues)):
        if numpy.linalg.norm(res_C[i]-res_CA[i]) > 2.5:
            print(residues[i],' c-ca',numpy.linalg.norm(res_C[i]-res_CA[i]))
            num_brk+=1
        if numpy.linalg.norm(res_N[i]-res_CA[i]) > 2.5:
            print(residues[i],' n-ca',numpy.linalg.norm(res_N[i]-res_CA[i]))
            num_brk+=1
