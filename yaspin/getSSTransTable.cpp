...
int main(int argc, char** argv)
{
    ...
    string sslables="CGHIDEF";
    float transTable[7][7];
    for (size_t i=0;i<7;i++)
        for (size_t j=0;j<7;j++)
            transTable[i][j]=1; // Laplace's rule of succession.
    
    string transSeg[7][7];
    for (size_t i=0;i<7;i++){
        for (size_t j=0;j<7;j++){
            transSeg[i][j]+=sslables[i];
            transSeg[i][j]+=sslables[j];
        }
    }
    ...
    for (size_t l=0;l<ind.size();l++){
        ...
        ////////////////////////////////////////////////////////////////////////
        for (size_t i=0;i<states7.length()-1;i++){
            string ts=states7.substr(i,2);
            for (size_t j=0;j<7;j++){
