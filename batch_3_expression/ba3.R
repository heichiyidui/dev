...
gs_report  <- "/home/kuang/dev/batch_3_expression/input_files/Sample_and_Control_Probe_Profile_FinalReport.txt"
gs_probe   <- "/home/kuang/dev/batch_3_expression/input_files/Sample_and_Control_Probe_Profile_FinalReport.txt"
gs_sample  <- "/home/kuang/dev/batch_3_expression/input_files/sample_table_Final_Report.txt"
gs_control <- "/home/kuang/dev/batch_3_expression/input_files/Sample_and_Control_Probe_Profile_FinalReport.txt"
anno_table <- "/home/kuang/dev/batch_3_expression/input_files/Sample_and_Control_Probe_Profile_FinalReport.txt"

# sample information FILE NAME must contain :
# Sample.ID,SEX,GROUPS,TISSUE,PHENOTYPE,Study_ID
pheno_file <- "/home/kuang/dev/batch_3_expression/input_files/pheno_info.txt"

# batch information
tech_pheno_file <- "/home/kuang/dev/batch_3_expression/input_files/batch_info.txt"
...
# use about 5G of memory now
# about 20 mins
control_data=read.table('../input_files/controlprobe.txt',
                         header=TRUE,sep='\t',check.names=FALSE)
head(control_data)

a=addControlData2lumi(control_data,eset_raw)
eset_raw=a
...
# WARNING!: The number of unique Study_Ids=[ 559 ] 
# WARNING!: The number of unique Study_Ids=[ 530 ] 
# WARNING!: The number of unique Study_Ids=[ 506 ] 
...
# Problem: ask for control data in the function removeSamples_eset_lumi
# TODO: fix this!

#######################################
# 5  update pData
head(eset_pheno_batch_merge)
save(eset_pheno_batch_merge, file = paste(out_dir, "/", project_name, ".eset_pheno_batch_merge.RData", 
    sep = ""))
...
new_pdata <- merge(old_pdata, eset_pheno_batch_merge, by.x = "sampleID", 
...
cat(" Number of chips with complete Phenotype and Batch data=[", n_expression_chips_with_data, 
    "]",  "\n")
# Number of chips with complete Phenotype and Batch data=[ 559 ]
...
write_expression_files(eset = eset_raw, outfile = paste(out_dir, "/", project_name, 
    ".eset_raw", sep = ""))
# Writing fData slot of eset [ ... BA3.eset_raw.fDat .txt ] 
# Then crashed! 
...
#######################################
# need to remove the 4 ids ?          #
#######################################

#  samples_to_remove <- c("9371242087_K","9534190041_I","9402444035_A","9534190031_I")
# a <- removeSamples_eset_lumi(eset = eset_raw, sampleRemove = samples_to_remove)
# eset_raw <- a 
...
# a <- removeSamples_eset_lumi(eset = eset_bg, sampleRemove = samples_to_remove)
# eset_bg <- a 
...
# skip the gender checking, the default one is problematic
...
# Total number of good probes =  3976
# LK: not too bad ? 3976 vs 4756
...
# remember to assign pData if reading from saved file 
...
ISAoutliers <- basic_sampleNetworkIterate(eset = eset_bg_log2_rsn_0, col_by_chip = 0,
    groups = "all", outfile = paste(out_dir, "/", project_name, ".eset_bg_log2_rsn_0_grp_all",
        sep = ""), IACthresh = 0.95, sd_thrs = iac_sd_thrs)
...
date_vars <- grep("DO", batch_var_names)  # which ones are dates
batch_var_names

multivariate_model_terms <- paste(batch_var_names, collapse = "+")
# multivariate_model_terms <- "Sentrix.Barcode+SampleSection+DOB+DOC+DOE+RIN+RNA_YIELD"
multivariate_model_terms <- "tech.Sentrix.Barcode+tech.SampleSection+tech.RIN"
...
#  WARNING!: SIGNIFICANT ASSOCIATION BETWEEN [  PC1  ] ~ [ tech.Sentrix.Barcode + tech.SampleSection + tech.RIN  ]. R2= 0.173  [ tech.RIN ] MIN_P= 1.92301e-17 . YOU MAY  ANT TO CORRECT FOR THIS BEFORE THE FINAL ANLYSIS  
#  Finding a best model by AIC in a Stepwise Algorithm  
# Start:  AIC=3387.48
# PC1 ~ tech.Sentrix.Barcode + tech.SampleSection + tech.RIN
#
#                        Df Sum of Sq    RSS    AIC
# - tech.Sentrix.Barcode 45     37543 362649 3352.8
# - tech.SampleSection   11     14185 339291 3387.1
# <none>                              325106 3387.5
# - tech.RIN              1     56939 382045 3467.1
#
# Step:  AIC=3352.77
# PC1 ~ tech.SampleSection + tech.RIN
#
#                        Df Sum of Sq    RSS    AIC
# - tech.SampleSection   11     15076 377725 3351.4
# <none>                              362649 3352.8
# + tech.Sentrix.Barcode 45     37543 325106 3387.5
# - tech.RIN              1     64689 427337 3433.8
#
# Step:  AIC=3351.38
# PC1 ~ tech.RIN
#
...
# 
# LK: tried different combinations, the best is RIN only
...
project_summary <- data.frame(project_dir = project_dir, project_name = project_name,
    ...
    n_good_samples = dim(eset_bg_log2_rsn_0)[2], n_unique_study_id_good_samples = length(unique(pData(eset_bg_log2_rsn_0)$Study_ID)))
