...
as.integer(cut(x, quantile(x, probs, na.rm=TRUE), include.lowest=TRUE))
}
##############
## var_gene ##
##############
...
gx_qc_plots_lumi <- function(eset, outfile ,do_pca=TRUE ) {
  ...
  colorvec <- labels2colors(as.character(pData(eset)$tech.Sentrix.Barcode))
...
pca_plot_lumi <- function(eset) {
  ...
  quant_numeric <- apply( numeric_tech,2,quantfun)
  ## colours
  batch_col <- apply(batch_pheno[,sel_batch],1,labels2colors) ## colours
  batch_col <- t(batch_col)
  quant_numeric_col <- apply(quant_numeric,2,numbers2colors)
  datColors <- cbind(chip_col,group_col,pheno_col,gender_col,tissue_col,batch_col,quant_numeric_col)
  #
...
if(gp_col=="chip") { colorvec <- labels2colors(as.character(pData(eset)$tech.Sentrix.Barcode)) }
...
negativeControl <- getControlData(eset)#,type='LumiBatch')
...
if(col_by_chip == 1) {   colorvec <- labels2colors(as.character(pData(eset)$tech.Sentrix.Barcode)) }
...
cat(" Groups [",group_list,"]","\r","\n")
	...
	write.table(FNC_DF[Z.K_outliers,],file=paste(outfile,".group.",mgroup,".SampleNetwork_Stats_Z.K_outliers.txt",sep=""),sep="\t",row.name=FALSE,quote=FALSE)
	## set colours by chip of GROUPS
	colorvec <- labels2colors(as.character(pData(eset[,pData(eset)$GROUPS==mgroup])$tech.Sentrix.Barcode))
