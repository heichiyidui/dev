...
int main(int argc,char**argv){
    ...
    for (size_t dom_index=0;dom_index<dom_ids.size();dom_index++){
        ...
        for (size_t i=0;i<mat_len ; i++){
            ...
            dom_cont_scores.push_back(norm_score2);
        }
        cout<<dom_id<<' '<<accumulate(dom_cont_scores.begin(), \
                                      dom_cont_scores.end(),0.0) \
                           / dom_cont_scores.size() <<'\n';
    }
